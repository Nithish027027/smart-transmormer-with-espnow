...
void setup() {
    Serial.begin(115220);
    Serial.println("NodeMCU Transmitter 1 Booting...");
...
void sendRegularPacket(float voltageValue) {
    ...
    dataPacket.current = 1.0; // Sends constant current value "1.0"
...
void loop() {
    ...
    if (currentVoltage < 100.0) {
        ...
        dataPacket.kWh = 0;
        dataPacket.special_char = 'a';
        
        esp_now_send(receiverAddress, (uint8_t *) &dataPacket, sizeof(dataPacket));
        Serial.print("CONTINUOUS ALERT 'a' SENT! Voltage: "); 
